...
|                                               C++ Libraries                                                                                  |
|----------------------------------------------------------------------------------------------------------------------------------------------|
| \<iostream\> - библиотека для ввода и вывода значений из консоли.                                                                            |
| \<string\> - библиотека для работы со строками и их преобразованием (std::string)                                                            |
| \<memory\> - библиотека для работы с умными указателями (то есть они сами себя уничтожают себя после окончания программы)                    |
| \<fstream\> - библиотека для работы с файлами (открытие файла, закрытие файла, получение данных из файла, запись данных в файл)              |
| \<cstring\> - библиотека для работы со строками и буфферами (операции со строками: копировать, складывать, узнавать длину, сравнивать)       |
| \<cctype\> - библиотека для проверки типов данных на схожесть                                                                                |
| \<cstdio\> - библиотека C функций(stdio.h) типа printf, puts, scanf, gets, fopen...                                                          |
| \<cstdlib\> - библиотека C функций(stdlib.h) типа malloc, rand, atoi, exit...                                                                |
| \<cstdint\> - библиотека типов данных с константным размером (uint8_t - это unsigned int но размер точно будет 8 байт)                       |
| \<limits\> - библиотека о максимальных и минимальных значениях типов данных, а также: проверка на то что тип данных целочисленный или нет, проверка на то что тип данных число, проверка на то что тип данных может быть бесконечностью, проверка на то что тип данных NaN. (std::numeric_limits\<int\>::max() - максимальное значение типа данных int) |
| \<сlimits\> - библиотека о максимальных и минимальных значениях типов данных, значения обьявленны с помошью `#define` (INT_MAX - максимальное значение типа данных int) |
...
чтобы выяснить где ошибка в твоём коде используй `gdb` (Позволяет посмотреть как выполняется код и посмотреть значения переменных) или `gcov` (Позволяет посмотреть какие участки кода задействованы). `gdb` - для того чтобы использовать этот дебаггер надо скомпилировать программу с флагом '-g' и использовать дебаггер на исполняемом файле; `gcov` - для того чтобы использовать этот дебаггер надо скомпилировать программу с '-c' флагом и с '--coverage' флагом и использовать дебаггер на исходной программе.
...
`class "имя"` - class он похож на struct, но в class можно указывать что ты можешь доставать из него а что останется только в нём. вобщем sturct по стандарту там все переменные public, а в class по стандарту там все переменные private.использование "class" более предпочтительно по сравнению с struct в C++ из за большего количества полезных свойств. пример кода как сделать class и struct и потом вызвать их:
```cpp
class Guard {
    public:
        std::string name;
...
в C/C++ размер массивов (`тип-данных имя[размер]`) расчитываеться по формуле: `sizeof(тип-данных) * кол-во-элементов`. И соответственно, чтобы узнать реальный размер массива надо не просто делать `sizeof(массив)`, а надо делать: `sizeof(массив)/sizeof(тип-данных-массива)`.
...
как работает то что компилятор языка написан на своём языке:
1. создаётся компилятор на другом языке который будет понимать твой язык программирования.
2. создаётся компилятор который написан на твоём языке программирования используя предыдущий компилятор.
...
отличия `std::thread` от `std::async` в том что. Во первых в `std::thread` нельзя получить то что вернула функция, а в `std::async` можно вызвать метод '.get()' и получить возвращаемое значение. Во вторых в `std::async` не обязательно ждать thread чтобы закончить программу, а в `std::thread` если не вызвать '.join()' или '.detach()' то программа аварийно завершится из за того что thread не был завершен и освобожден.

многие не знали что `std::ofstream` и `std::ifstream` позволяют записать в файл данные с помошью знака `<<` и получить данные с помошью знака `>>`.
...
для чего нужны `.h` и `.hpp` файлы в C++(header файлы): они нужны во сновном чтобы не писать прототип функций (чтобы компилятор понимал что эта функция будет определена в другом файле) вручную, а вместо этого можно было бы просто сделать `#include "имя_файла.h"` и тогда все что написано в header файле будет перенесено в место где включают его. НЕ ЗАБУДЬ ПОСТАВИТЬ `#pragma once` в начало header файла а то он будет включаться множество раз и будет ошибка.

разница между `"\x3a"` и `0x3a` в том что в первом случаи при выводе будет символ ':' (превращение в char) а во втором случаи будет число '58' (нет превращения в другой тип данных). Если что `3a` это hex число а в dec системе счисления оно бдует числом `58`.
...
чтобы узнать предопределённые макросы для C++ можно воспользоваться командой: `g++ -dM -E -x c++ - < /dev/null` 
```
...
`std::move` - функция C++ для преобразования из lvalue в rvalue (из переменной в просто значение, пример: `int a = 1; // this is lvalue` `std::move(a) // this is rvalue now` )

Асинхронное програмирование в C++: вариант 1: `std::thread th(func);` а затем `pr.detach()` "func" будет тогда выполняться параллельно. вариант 2: `auto th = std::async(std::launch::async, func);` это выполнит "func" параллельно (предпочтительнее при выборе из этих двух вариантов).

Препроцессор распознает следующие директивы:

#define - эта директива умеет: `#define VAL 10` - работать как константа, `#define LENGTH(val) std::cout << strlen(val);` - работать как функция-макрос-лямбда. Чтобы было удобнее писать большие макросы можно поставить знак `\` и тогда можно написать предыдуший макрос вот так: 
```cpp
#define LENGTH(val) \
std::cout << strlen(val);
```

#elif - else if но для проверки значений `#define` (ставится только с `#if`)

#else - else для препроцессора (ставится только с `#if`)

#endif - конец препроцессорных выражений 

#error

#if - if но для проверки значений `#define` (ставится `#endif` в конце)

#ifdef - if но для проверки на то что макрос `#define` определён (ставится `#endif` в конце)

#ifndef - if но для проверки на то что макрос `#define` не определён (ставится `#endif` в конце)
