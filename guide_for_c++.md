...
Асинхронное програмирование в C++: `std::thread th(func);` а затем `pr.detach()`, "func" будет тогда выполняться параллельно, и `auto th = std::async(std::launch::async, func);` это тоже выполнит "func" параллельно (предпочтительнее при выборе из этих двух вариантов).
