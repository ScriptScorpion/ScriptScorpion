...
#define - эта директива умеет: `#define VAL 10` - работать как константа, `#define LENGTH(val) std::cout << strlen(val);` - работать как функция-макрос-лямбда. Чтобы было удобнее писать большие макрос можно поставить знак `\` и тогда можно написать предыдуший макрос вот так: 
```
#define LENGTH(val) \
std::cout << strlen(val);
```
