...
JS Переход, если знаковый

JNS Переход, если беззнаковый

JO Переход, если переполнение

JNO Переход, если нет переполнения

JC Переход, если переполнение при сложении

JNC Переход, если переполнения нету при сложении

JP Переход, если количество единиц в младшем байте результата чётно

JNP Переход, если количество единиц в младшем байте результата нечётно
...
$$ - в NASM означает позицию начала текущей секции( .data, .text, ...)

[] - в NASM означает значение по адресу, пример: `mov rax, var`('var' - переменная обьявлённая в `section .data`) - в этом примере мы передаём адрес переменной var в регистр rax, а если сделать так: `mov rax, [var]` - то тогда мы передаём значение по адрессу переменной var в регистр rax.
...
в ассемблере стэк(stack) растёт вниз, а куча(heap) растёт вверх. 
Чтобы положить значение в стэк надо вычесть из указателя на стэк количество битов которые ты хочешь положить туда и затем можно положить значение в память по аддресу указателя на стэк, а когда это значение больше не нужно, надо востановить состояние стэка в нормальное положение, то есть надо прибавить к указателю на стэк количество битов которых надо было для того чтобы положить значение туда. 
Для того чтобы положить значение в кучу на Linux используеться `brk` syscall, для этого надо сначало получить аддрес текущего окончания програмы с помощью первого вызова `brk`, затем добавить к указателю на память в первом syscall количество битов которые ты хочешь положить туда, и затем сделать ещё один вызов `brk` чтобы приминить это, после этого ты можешь положить значение в память по аддресу с первого syscall, а когда это значение больше не нужно, надо востановить состояние кучи в нормальное положение, то есть надо вычесть из аддреса второго syscall количество битов которых надо было для того чтобы положить значение туда, и затем вызвать ещё один вызов `brk` чтобы приминить изменения.
...
для чего можно использовать битовые операции: 1) xor - использовать можно с самим значение чтобы быстро сбросить значение до нуля `xor rax, rax` или можно использовать для того чтобы положить число в другое число и убрать биты у числа которые равны битам твоего числа `xor rax, 10`. 2) or - использовать можно чтобы положить число в другое число но при этом не трогать биты другого числа `or rax, 10`. 3) and - использовать можно для того чтобы получить определенное количество битов у значения `and rax, 3` чтобы узнать последние 2 бита. 
